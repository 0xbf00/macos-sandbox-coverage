...
def collect_sb_traces(bundle: Bundle, outdir: str, timeout: int) -> bytes:
    ...
    try:
        return pid, subprocess.check_output(["log", "show",
                         "--start", start,
                         "--end", end,
                         "--style", "json",
...
def gather_logs(app_path: str, outdir: str, timeout=60):
    ...
    pid, traces = collect_sb_traces(bundle, outdir, timeout)
    if not traces:
        logger.error("Unable to collect sandbox logs for app {}".format(app_path))
        return

    return pid, json.loads(traces)